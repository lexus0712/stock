...
def MACD(data):    # Data is going to be our OHLCV
    ...
    df2['fast_line'] = talib.SMA(df2['close'], timeperiod=int(sys.argv[4]))
    df2['slow_line'] = talib.SMA(df2['close'], timeperiod=int(sys.argv[5]))
    # 计算差离值 (DIF)
    df2['dif_line'] = df2['fast_line'] - df2['slow_line']
    # 计算信号线 (DEA)，使用 DIF 的9日简单移动平均
    df2['signal_line'] = talib.SMA(df2['dif_line'], timeperiod=int(sys.argv[6]))
